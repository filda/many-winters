...
- **Step 9 — Seasons and first winter** (partial): `WorldState.CurrentSeason` cycles Spring→Summer→Autumn→Winter derived from tick count (75 ticks/season, shown in the Godot UI next to the tick counter) — no extra state to save/load since it's computed straight from the clock. Winter is the actual pressure: hunger accrues at double rate (`WorldState.Advance` applies each tick's own seasonal rate rather than a flat batch formula, so a batch spanning a season boundary charges each tick correctly), and a `ResourceDefinition` can set a `WinterYieldMultiplier` — the four food resources drop to 40% yield in winter while `wood` is unaffected, so winter is harder but still survivable with enough people/techniques/tools, not an automatic death sentence. Resource nodes also regrow: each `ResourceDefinition` can set a `RegenPerTick`, and a `ResourceNode` regrows back toward the amount it was originally spawned with (`MaxAmount`) — paused during winter, same reasoning as the yield cut — so a node survives being harvested across multiple winters instead of only ever depleting. `WinterSurvivalMilestoneTests` proves winter's pressure is real (stopping gathering right as it begins is fatal within it) as distinct from Step 6's "never gather at all" baseline. Weather severity and clothing/insulation are still deferred.
