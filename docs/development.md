...
`print prologue` prints the inscription the game shows over a band's arrival, and once its line has ended `print epitaph` prints the one over its end (see `Prologue` and `Epitaph` in `ManyWinters.Core.Continuity`) - the quickest way to read their wording over many different bands without playing each one out:

```powershell
dotnet run --project src/ManyWinters.Tools/SimulationRunner -- generate create 4 print prologue simulate 100 print epitaph
```
...
- **`application/boot_splash/image`** is `res://Content/splash/title_page.png`, generated by `art/generate_splash.py`: an engraved title page shown instead of the Godot logo for the whole synchronous world build in `Main._Ready`. It used to be set in Georgia Bold and Constantia Italic read from `C:/Windows/Fonts`, which are Microsoft's and could neither be committed nor exported, and made the image reproducible only on Windows; it now draws with the game's own faces out of the repository. Those faces (`Ui/InscriptionFont.cs`) are two open ones under `Content/fonts/`: **IM Fell English** for titles (`im-fell-english/`, one regular weight, old-style figures — never used below title size) and **Vollkorn** for everything else (`vollkorn/`, Regular and Bold, lining figures; the Italic is there for the title page's subtitle). **Every font in the game must be SIL Open Font License 1.1** (or as free): it may be bundled into a commercial build, redistribution only asks that the copyright and licence text travel with the file — hence the `OFL.txt` beside each — and a modified font must respect the Reserved Font Names in that text. To judge a candidate, render a specimen sheet, not a headline: a large title, a 14–18 px paragraph, a person's name line, a resource counter with numbers, a button, a tooltip and Czech text with diacritics — that is what ruled out Averia Serif Libre (missing Czech glyphs) and the old-style single-weight faces as text fonts. The inspector deliberately stays in the engine's default font: it is a debugging aid, not part of the game's face. `boot_splash/bg_color` matches `default_clear_color` below so the letterboxed margins and the first rendered frame share one colour.
...
The repository-wide build and development tasks use Cake Frosting, a compiled C# console application. It keeps the same task graph for local work and GitHub Actions:

```powershell
# Show the available tasks
dotnet run --project build/ManyWinters.Build.csproj -- --help
