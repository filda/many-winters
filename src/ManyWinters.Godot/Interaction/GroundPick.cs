...
// far bigger than its drawn silhouette (see SpriteVisibleExtent): a click into a conifer box's
// transparent corner arrives with the hit point on that box, meters up and shifted toward the
// camera - tens of meters off the ground under the cursor at a shallow tilt. Every view is an
// Area3D and the only body is TerrainRenderer's ground StaticBody3D, so a ray that skips areas
// can only land on the ground.
//
// ProjectRayOrigin/ProjectRayNormal rather than camera position plus a direction: that
// reconstruction only fits a perspective camera and breaks after FreeCameraRig.ToggleProjection.
