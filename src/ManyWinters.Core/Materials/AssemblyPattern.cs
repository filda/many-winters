...
//
// Material is deliberately left out. An axe is an axe whether its head is stone or flint - the
// substance stays an adjective in front of the name rather than part of it. So the first band
// to lash a wedge to a shaft coins one word, and every later one of those is that word for
// free, including ones made of substances nobody had then heard of.
//
// The signature is a string rather than a type of its own because the only thing anyone does
// with it is look a name up by it, and it has to survive a save unchanged.
